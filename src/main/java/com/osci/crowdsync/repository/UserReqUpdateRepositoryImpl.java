                        ...
                        (sysUser.useYn.eq("Y").and(sysUser.corpCode.eq("OSC"))
                                .and(
                                        (sysUser.deptName.ne(updatedUser.deptName).or(sysUser.posName.ne(updatedUser.posName)))
                                                .or
                                        (crowdUsernameCustom.userCustomName.isNotNull())
                                                .or
                                        (updatedUser.userId.isNull())
                                )).and(crowdUsernameCustom.chkYn.isNull().or(crowdUsernameCustom.chkYn.eq("N")).or(crowdUsernameCustom.chkYn.eq("Y").and(crowdUsernameCustom.userCustomName.ne(updatedUser.displayName).or(updatedUser.displayName.isNull()))))
                )
                .fetch();
    }
